...
- Library shows one section per configured category only (Embedding/VAE/etc. appear once you add their folder); entries come from server `/api/library` rows plus local scan records. Categories accept multiple paths (`dir_<Type>`, `dir_<Type>__N`); server and browser scans both recurse into subfolders. Folder categories ≠ Civitai types — `lib/categories.ts` maps API types (TextualInversion→Embedding, …) for grouping; scans record the folder category.
...
- Start everything with `./start.sh` (venv + backend :5000, then Vite :5173; Ctrl+C stops both; `--backend-only` / `--frontend-only` / `--no-install` flags).
