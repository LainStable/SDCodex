...
- Library shows one section per configured category only (Embedding/VAE/etc. appear once you add their folder); entries come from server `/api/library` rows plus local scan records. Categories accept multiple paths (`dir_<Type>`, `dir_<Type>__N`); server and browser scans both recurse into subfolders.
